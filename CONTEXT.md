...
- **Trusted publishing**: npm's OIDC-based publish authentication — a registered GitHub workflow publishes with no npm token anywhere. Only configurable on a package that already exists, which forces the bootstrap's one manual publish.
- **Provenance attestation**: the registry-attached, verifiable statement of where a published tarball was built (repo, workflow, commit). Generated automatically by trusted publishing only; a manual local publish can never carry one.
